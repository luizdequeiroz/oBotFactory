using Extensions.ObjectManager;
    ...
    public class DynamicController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateAsync(ServiceType serviceType, dynamic entity)
        {
            try
            {
                ...
                return Success(registerCreated, $"Registro de {serviceType.ToDescriptionString()} criado com sucesso!");
            }
            catch (Exception ex)
            {
                return Error(ex);
        ...
        public async Task<IActionResult> ReadAllAsync(ServiceType serviceType)
        {
            try
            {
                var regiters = await serviceSwitch.Case(serviceType).GetAllAsync();

                if (regiters.Count == 0)
                    return Warning($"Nenhum registro de {serviceType.ToDescriptionString()} encontrado!");

                return Success(regiters);
            }
            catch (Exception ex)
            {
                return Error(ex);
